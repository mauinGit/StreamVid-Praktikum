...
<div class="sv-auth-container" style="min-height:100vh; display:flex; align-items:center; justify-content:center; position: fixed; top: 0; left: 0; right: 0; bottom: 0; z-index: 9999; background: url('{{ asset('img/welcome.png') }}') no-repeat center center; background-size: cover; padding: 20px;">
    ...
    {{-- Register Form Card --}}
    <div style="position: relative; z-index: 2; background: rgba(20,20,20,0.95); backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.1); border-radius: 16px; padding: 48px 40px; width: 100%; max-width: 420px; max-height: 90vh; overflow-y: auto; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.7);">
        ...
        <div style="margin-bottom: 32px; text-align: center;">
            <h2 style="font-size: 1.6rem; font-weight: 800; margin: 0 0 8px; color: #fff;">Buat Akun Baru</h2>
        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <div class="sv-form-group" style="margin-top: 24px; margin-bottom: 24px;">
                <label style="display: flex; align-items: flex-start; gap: 10px; cursor: pointer; color: var(--sv-text-muted); font-size: 0.8rem; line-height: 1.4;">
                    ...
                    <span>Saya menyetujui <a href="{{ route('terms') }}" target="_blank" style="color: var(--sv-accent); text-decoration: none;">Syarat & Ketentuan</a> dan <a href="{{ route('terms') }}" target="_blank" style="color: var(--sv-accent); text-decoration: none;">Kebijakan Privasi</a> StreamVid</span>
                </label>
                @error('terms')
            ...
            <button type="submit" class="sv-btn sv-btn-primary" style="width: 100%; padding: 14px; font-size: 1rem; font-weight: 600;">Daftar</button>
        </form>
...
<style>
    ...
    /* Hide scrollbar for the form card */
    .sv-auth-container > div:last-child::-webkit-scrollbar { display: none; }
    .sv-auth-container > div:last-child { scrollbar-width: none; -ms-overflow-style: none; }
